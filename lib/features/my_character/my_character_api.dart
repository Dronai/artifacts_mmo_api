...
class MyCharacterApi {
  ...
  Future<RecyclingDataSchema> actionRecycling(
    String characterName,
    SimpleItemSchema item,
  ) async {
    final String path = r'/my/{name}/action/recycling'.replaceAll(
      '{name}',
      characterName,
    );
    Map<String, String>? queryParams;
    ...
    if (responseBody.isNotEmpty) {
      return RecyclingResponseSchema.fromJson(jsonDecode(responseBody)).data;
    } else {
      throw ApiException(response.statusCode, 'Response body is empty');
    }
  }

  Future<DeleteItemSchema> actionDeleteItem(
    String characterName,
    SimpleItemSchema item,
  ) async {
    final String path = r'/my/{name}/action/delete'.replaceAll(
      '{name}',
      characterName,
    );
    Map<String, String>? queryParams;
